...
Commit messages are validated with Conventional Commits via commitlint; use prefixes such as `feat:`, `fix:`, `docs:`, and `chore:`. Keep commits scoped to one logical change. Unless explicitly instructed otherwise, create pull requests against `Mayimg/RiichiEnv`. Pull requests should describe the behavior change, list the commands you ran, link related issues, and include screenshots or short recordings for UI or visualization updates.
